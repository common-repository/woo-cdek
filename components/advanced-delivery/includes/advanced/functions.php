 ...
 *
 */
function wdad_html_field( $args = array() ) {
	...
	switch ( $type ) :
		...
		case 'select' :
			...
			foreach ( $options as $index => $values ) :
				...
				if ( ! is_array( $values ) ) :
					?>
					<option
					value='<?php echo esc_attr( $index ); ?>' <?php selected( $index, $value ); ?>><?php echo esc_attr( $values ); ?></option><?php
				else :
					?>
					...
					foreach ( $values as $k => $v ) :
						?>
						<option
						value='<?php echo esc_attr( $k ); ?>' <?php selected( $k, $value ); ?>><?php echo esc_attr( $v ); ?></option><?php
					endforeach;
					?></optgroup><?php
				endif;
		...
		case 'hook' :
			...
			do_action( 'wdad_advanced_condition_value_field_type_' . $args['type'], $args );
			break;
