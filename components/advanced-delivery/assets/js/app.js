...
function deliverySearch(method, locality, geo, callback) {
    ...
    if (window.advancedSearchAjax && window.advancedSearchAjax.readyState < 4) {
        window.advancedSearchAjax.abort();
    }

    window.advancedSearchAjax = jQuery.ajax({
        url: wWoocommerceCdekDeliveryIntegration.variables.ajaxurl,
        method: 'POST',
        data: {
            action: "wdad_delivery_search",
            method: method,
            locality: locality,
            geo: geo
        },
...
function renderDelivery(itemData) {
    ...
    jQuery(".woocommerce-advanced-delivery").remove();
        ...
        jQuery("[data-type='locality" + locality + "']").selectWoo({
            ...
            ajax: {
                ...
                data: function (params) {
                    return {
                        action: "wd-advanced-delivery-city-autocomplete",
...
function updateData(event) {
    jQuery(document).find('#wdad_btn_delivery_method').hide();
    jQuery(document).find('.woocommerce-advanced-delivery').remove();
    ...
    window.advancedWidget = new ISDEKWidjet({
        ...
        onLoad: function () {
            jQuery(document).find('#wdad_btn_delivery_method').show();
            jQuery(document).find('#wdad_widget_loader').hide();
        },
    });

    jQuery('#wdad_widget_loader').show();
}
...
function hideNonYDFields() {
    ...
    var ydElements = [
        '#wdad_city_field', '#wdad_ship_city_field'
    ];
    var nativeElements = [
    ...
    } else if (method.startsWith('wbcr_advanced_delivery')) {
...
jQuery(document).on('click', '#wdad_btn_delivery_method', function (e) {
    e.preventDefault();
    window.advancedWidget.open();
});
...
jQuery(document).ready(function () {
    ...
    if (typeof jQuery(document).selectWoo === 'function') {
        ...
        selectWooLocality();
        jQuery('#wdad_city').on('change', updateData);
        jQuery('#wdad_ship_city').on('change', updateData);
        if (jQuery('#ship-to-different-address-checkbox').prop('checked')) {
