...
class Activation extends \Wbcr_Factory437_Activator {
	...
	public function activate() {
		if ( ! is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			die( __( 'The plugin requires Woocommerce', 'wd-cdek-delivery' ) );
		}

		if ( ! class_exists( 'WC_Shipping_Method' ) ) {
			die( __( 'WC_Shipping_Method class not found', 'wd-cdek-delivery' ) );
		}

		$package = Plugin::app()->premium->get_package_data();
		if ( empty( $package ) ) {
			$plugin_data = get_file_data( WP_PLUGIN_DIR . '/' . WDCD_PLUGIN_BASE, [
				'Version'          => 'Version',
				'FrameworkVersion' => 'FrameworkVersion'
			] );
			$package     = [
				'basename'          => WDCD_PLUGIN_BASE,
