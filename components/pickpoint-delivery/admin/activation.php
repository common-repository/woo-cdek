...
namespace WPickpoint\Delivery\Pickpoint;
...
class Activation extends \Wbcr_Factory437_Activator {
	...
	public function activate() {
		if ( ! is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			die( __( 'The plugin requires Woocommerce', 'pickpoint-delivery' ) );
		}

		if ( ! class_exists( 'WC_Shipping_Method' ) ) {
			die( __( 'WC_Shipping_Method class not found', 'pickpoint-delivery' ) );
		}

		$package = Plugin::app()->premium->get_package_data();
		if ( empty( $package ) ) {
			$plugin_data = get_file_data( WP_PLUGIN_DIR . '/' . WPICKPOINT_PLUGIN_BASE, [ 'Version' => 'Version', 'FrameworkVersion' => 'FrameworkVersion' ] );
			$package     = [
				'basename'          => WPICKPOINT_PLUGIN_BASE,
