...
class Plugin extends \Wbcr_Factory437_Plugin {
    ...
    public function plugins_loaded() {
        ...
        if ( $this->is_premium() && is_plugin_active( 'woocommerce/woocommerce.php' ) && class_exists( 'WC_Shipping_Method' ) ) {
            require_once( WDPK_PLUGIN_DIR . '/libs/delivery/base/boot.php' );
    ...
    public function add_plugin_action_link( $actions, $plugin_file, $plugin_data ) {
        ...
        if ( $this->is_premium() ) {
            ...
            $license_link = '<a href="' . $this->getPluginPageUrl( 'license' ) . '" style="color: #FF5722;font-weight: bold;">' . __( 'License', WDPK_PLUGIN_SLUG ) . '</a>';
    ...
    public function add_plugin_row_link( $links, $file ) {
        ...
        $docs_link = sprintf( "<a href='".$this->get_support()->get_docs_url( true, 'plugins_page' )."'>%s</a>", __( 'Documentation', WDPK_PLUGIN_SLUG ) );
        $links[]   = $docs_link;
    ...
    private function register_pages() {
        self::app()->registerPage( 'WDPK_LicensePage', WDPK_PLUGIN_DIR . '/admin/pages/class-pages-license.php' );
    }
}
