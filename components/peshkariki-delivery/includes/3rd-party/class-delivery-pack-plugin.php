...
class Plugin {
	...
	public function __construct( $plugin_path ) {
		...
		if ( is_admin() ) {
			...
			require( WDPK_PLUGIN_DIR . '/admin/boot.php' );
			require( WDPK_PLUGIN_DIR . '/admin/ajax/check-license.php' );
	...
	public function plugins_loaded() {
		...
		if ( class_exists( 'WC_Shipping_Method' ) ) {
			...
			require_once( WDPK_PLUGIN_DIR . '/includes/Pehskariki/load.php' );
	...
	protected function init_activation() {
		include_once( WDPK_PLUGIN_DIR . '/admin/activation.php' );
	...
	public function admin_notices_configured() {
		...
		if ( $section !== 'wbcr_peshkariki_delivery' ) {
			$text = sprintf(
				'<strong>%s</strong>: %s',
				__( 'Peshkariki Delivery', 'wd-peshkariki-delivery' ),
				sprintf(
					'%s <a href="%s">%s</a>',
					__( 'The plugin does not work and requires configuration, please go to', 'wd-peshkariki-delivery' ),
					get_admin_url( null, \sprintf( '/admin.php?page=wc-settings&tab=shipping&section=%s',
						Peshkariki::SHIPPING_DELIVERY_ID ) ),
					__( 'plugin settings', 'wd-peshkariki-delivery' )
				)
			);
...
NOTICE;
	...
	public function admin_notices_dev() {
		...
		if ( Peshkariki::is_dev_mode() ) {
			$text = sprintf(
				'<strong>%s</strong>: %s',
				__( 'Peshkariki Delivery', 'wd-peshkariki-delivery' ),
				__( '<strong>ATTENTION!</strong> The plugin works in developer mode', 'wd-peshkariki-delivery' )
			);
...
NOTICE;
	...
	public function add_plugin_action_link( $actions, $plugin_file, $plugin_data ) {
		if ( false === strpos( $plugin_file, WDPK_PLUGIN_BASE ) ) {
			return $actions;
		}

		if ( $this->is_premium() ) {
			//$settings_link = sprintf( "<a href='%s'>%s</a>", admin_url( "admin.php?page=wc-settings&tab=shipping&section=" . Yandex::SHIPPING_METHOD_ID ), __( 'Settings', 'wd-peshkariki-delivery' ) );
			//array_unshift( $actions, $settings_link );
		} else {
			$license_link = '<a href="' . $this->app()->getPluginPageUrl( 'license' ) . '" style="color: #FF5722;font-weight: bold;">' . __( 'License', WDPK_PLUGIN_SLUG ) . '</a>';
	...
	public function add_plugin_row_link( $links, $file ) {
		if ( false === strpos( $file, WDPK_PLUGIN_BASE ) ) {
			return $links;
		}

		$docs_link = sprintf( "<a href='" . $this->app()->get_support()->get_docs_url( true, 'plugins_page' ) . "'>%s</a>", __( 'Documentation', WDPK_PLUGIN_SLUG ) );
		$links[]   = $docs_link;
