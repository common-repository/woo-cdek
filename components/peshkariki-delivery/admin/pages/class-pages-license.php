 ...
 */
class WDPK_LicensePage extends Wbcr_FactoryPages436_AdminPage {
    ...
    public function __construct( Wbcr_Factory437_Plugin $plugin ) {
        ...
        $this->menu_title    = __( 'Peshkariki Delivery', WDPK_PLUGIN_SLUG );
        $this->page_title    = __( 'License of Peshkariki Delivery Plugin', WDPK_PLUGIN_SLUG );
    ...
    public function assets( $scripts, $styles ) {
        ...
        $this->styles->add( WDPK_PLUGIN_URL . '/admin/assets/css/license-manager.css' );
        $this->scripts->add( WDPK_PLUGIN_URL . '/admin/assets/js/license-manager.js' );
    }
    ...
    public function indexAction() {
        ?>
        <div id="wdpk-license-wrapper" class="wbcr-factory-content"
             data-loader="<?php echo WDPK_PLUGIN_URL . '/assets/img/loader.gif'; ?>"
             data-plugin-name="<?php echo esc_attr( $this->plugin_name ); ?>"
             data-nonce="<?php echo wp_create_nonce( "wdpk_license_activate" ) ?>">
    ...
    protected function render_learnmore_section() {
        if ( $this->is_premium ):
            ...
            <p style="margin-top: 10px;">
		        <?php printf( __( '<a href="%s" target="_blank" rel="noopener">Lean more</a> about the premium version and get the license key to activate it now!', 'wd-peshkariki-delivery' ), $this->plugin->get_support()->get_pricing_url( true, 'license_page' ) ); ?>
            </p>
        <?php else: ?>
            <p style="margin-top: 10px;">
		        <?php printf( __( 'Can’t find your key? Go to <a href="%s" target="_blank" rel="noopener">this page</a> and login using the e-mail address associated with your purchase.', 'wd-peshkariki-delivery' ), "https://users.freemius.com" ) ?>
            </p>
        <?php endif;
    }
    ...
    public function show_license_form( $notice = false ) {
        ...
        <div id="license-manager"
            ...
            <div>
                <h3><?php printf( __( 'Activate %s', 'wd-peshkariki-delivery' ), $this->plugin->getPluginTitle() ) ?></h3>
		    ...
		    <?php if ( is_wp_error( $notice ) ) : ?>
                <div class="license-message <?= $this->get_license_type() ?>-license-message">
                    <div class="alert <?php echo esc_attr( $notice->get_error_code() ); ?>">
                        <h4 class="alert-heading"><?php _e( $notice->get_error_message(), 'wd-peshkariki-delivery' ) ?></h4>
                    </div>
                </div>
		    <?php endif; ?>
            <div class="onp-container">
                    ...
                    <?php if ( $this->get_license_type() == 'free' ): ?>
                        <a href="<?php echo $this->plugin->get_support()->get_pricing_url( true, 'license_page' ); ?>"
                           class="purchase-premium" target="_blank" rel="noopener">
                            ...
                            <?php _e( 'Buy Premium', 'wd-peshkariki-delivery' ) ?>
                            </span>
                        </a>
                        <p>&nbsp;</p>
                    <?php endif; ?>
                        ...
                        <div class="license-details-block <?= $this->get_license_type() ?>-details-block">
                            <?php if ( $this->is_premium ): ?>
                                <a data-action="deactivate" href="#"
                                   class="btn btn-default btn-small license-delete-button wdpk-control-btn">
		                            <?php _e( 'Delete Key', 'wd-peshkariki-delivery' ) ?>
                                </a>
                                <a data-action="sync" href="#"
                                   class="btn btn-default btn-small license-synchronization-button wdpk-control-btn">
		                            <?php _e( 'Synchronization', 'wd-peshkariki-delivery' ) ?>
                                </a>
                            <?php endif; ?>
                            <h3>
                            ...
                            <div class="license-key-description">
                                <?php if ( $this->is_premium ): ?>
                                    <p><?php _e( 'Сommercial license, only to the premium add-on to this free plugin. You cannot distribute or modify the premium add-on. But free plugin is a GPLv3 compatible license allowing you to change and use this version of the plugin for free.', 'wd-peshkariki-delivery' ) ?></p>
                                <?php endif; ?>
                                <?php if ( $this->is_premium && $this->premium_has_subscription ): ?>
                                    <p class="activate-trial-hint">
		                                <?php _e( 'You use a paid subscription for the plugin updates. In case you don’t want to receive paid updates, please, click <a data-action="unsubscribe" class="wdpk-control-btn" href="#">cancel subscription</a>', 'wd-peshkariki-delivery' ) ?>
                                    </p>
                                <?php endif; ?>

	                            <?php if ( $this->get_license_type() == 'trial' ): ?>
                                    <p class="activate-error-hint">
			                            <?php printf( __( 'Your license has expired, please extend the license to get updates and support.', 'wd-peshkariki-delivery' ), '' ) ?>
                                    </p>
	                            <?php endif; ?>
                            </div>
                            <table class="license-params" colspacing="0" colpadding="0">
                                <tr>
                                    <td class="license-param license-param-days">
                                        ...
                                        <span class="license-value-name"><?php _e( 'plan', 'wd-peshkariki-delivery' ) ?></span>
                                    </td>
			                        <?php if ( $this->is_premium ) : ?>
                                        <td class="license-param license-param-sites">
                                        <span class="license-value">
                                            <?php echo esc_attr( $this->premium_license->get_count_active_sites() ); ?>
                                            <?php _e( 'of', 'wd-peshkariki-delivery' ) ?>
                                            <?php echo esc_attr( $this->premium_license->get_sites_quota() ); ?></span>
                                            <span class="license-value-name"><?php _e( 'active sites', 'wd-peshkariki-delivery' ) ?></span>
                                        </td>
			                        <?php endif; ?>
                                    ...
                                    <?php if ( $this->is_premium ): ?>
                                        <td class="license-param license-param-days">
                                            <?php if ( $this->get_license_type() == 'trial' ): ?>
                                                <span class="license-value"><?php _e( 'EXPIRED!', 'wd-peshkariki-delivery' ) ?></span>
                                                <span class="license-value-name"><?php _e( 'please update the key', 'wd-peshkariki-delivery' ) ?></span>
                                            <?php else: ?>
                                                <span class="license-value">
													<?php
													...
													?>
                                                <small> <?php _e( 'day(s)', 'wd-peshkariki-delivery' ) ?></small>
                                             </span>
                                                <span class="license-value-name"><?php _e( 'remained', 'wd-peshkariki-delivery' ) ?></span>
                                            <?php endif; ?>
                                        </td>
                                    <?php endif; ?>
                                </tr>
                ...
                <div class="license-input">
                    <form action="" method="post">
			            <?php if ( $this->is_premium ): ?>
                    <p><?php _e( 'Have a key to activate the premium version? Paste it here:', 'wd-peshkariki-delivery' ) ?><p>
		            <?php else: ?>
                        <p><?php _e( 'Have a key to activate the plugin? Paste it here:', 'wd-peshkariki-delivery' ) ?>
                        <p>
				            <?php endif; ?>
                            <button data-action="activate" class="button button-primary wdpk-control-btn" type="button"
                                    id="license-submit">
					            <?php _e( 'Submit Key', 'wd-peshkariki-delivery' ) ?>
                            </button>
